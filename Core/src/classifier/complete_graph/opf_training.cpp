...
Model* OPFTraining::train(opf::Distance distance, Patterns patterns){
    ...
    PriorityQueue Q(intial_costs.begin(),initial_costs.end(),PriorityQueue::Type::MIN);
    ...
    while(!Q.empty()){
        ...
        model.node[p].set_cost(Q.get_cost(p));
        ...
        for(const int *q = Q.begin(); q != Q.end(); ++q){
            ...
            weight = distance(model.node[p].get_feature_vector(), model.node[*q].get_feature_vector());

            tmp = max(weight,Q.get_cost(p));

            if(weight < Q.get_cost(*q)){
                Q.update(*q, weight);
                model.node[*q].set_predecessor(p);
                model.node[*q].set_class_value(model.node[p].get_class_value());
            }
        }
    }
    return model;
